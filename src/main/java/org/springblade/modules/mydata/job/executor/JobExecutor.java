...
public class JobExecutor implements ApplicationRunner {
    ...
    public void executeOnce(Long id) {
        TaskInfo taskInfo = jobCache.getTask(id.toString());
        if (taskInfo == null) {
            Task task = taskService.getById(id);
            if (task == null) {
                return;
            }
            taskInfo = this.build(task);
            taskInfo.setTimes(1);
        }
        executeJob(taskInfo);
    }
     ...
     * @param taskInfo 任务
     */
    public void retryJob(TaskInfo taskInfo) {
        cacheJob(taskInfo, true);
    }
     ...
     * @param taskInfo job
     */
    public void cacheJob(TaskInfo taskInfo, boolean isRetry) {
        // 清空任务时间信息
        ...
        while (i < MdConstant.TASK_MAX_FAIL_COUNT) {
            try {
                ...
                String period = isRetry ? MdConstant.TASK_FAILED_PERIOD : taskInfo.getTaskPeriod();
                // 计算Job的下次执行时间
                calculateNextRunTime(taskInfo, period);

                taskInfo.appendLog("预计开始时间：{}，距离时长：{}秒", DateUtil.formatDateTime(taskInfo.getNextRunTime()), DateUtil.between(taskInfo.getStartTime(), taskInfo.getNextRunTime(), DateUnit.SECOND));
    ...
    public void completeJob(TaskInfo taskInfo) {
        ...
        // 任务执行次数
        int executeCount = taskInfo.getExecuteCount();
        // 任务执行结果
        int executeResult = taskInfo.getExecuteResult();
        // 取出任务可执行次数
        ...
        if (executeResult == MdConstant.TASK_RESULT_FAILED && executeCount >= MdConstant.TASK_MAX_FAIL_COUNT) {
            ...
            // 记录终止
            taskInfo.appendLog("任务失败达到{}次，将终止且不再执行", executeCount);
            // 删除任务缓存
        ...
        if (MdConstant.TASK_RESULT_SUCCESS == taskInfo.getExecuteResult()) {
            ...
            if (times > 0) {
                ...
                // 可执行次数为0，则结束任务
                taskService.stopTask(taskInfo.getId());
            }
        }
        // 任务失败
        else {
    ...
    private void executeJob(TaskInfo taskInfo) {
        ...
        taskInfo.setExecuteCount(taskInfo.getExecuteCount() + 1);
    ...
    private TaskInfo buildSubTaskJob(TaskInfo parentTaskInfo, Task subTask) {
        // 订阅任务 是消费数据
        TaskInfo subTaskInfo = build(subTask);
        // 订阅任务 执行1次
        subTaskInfo.setTimes(1);
        // 复用数据批次编号
     ...
     * @param taskInfo 定时任务
     */
    private void calculateNextRunTime(TaskInfo taskInfo, String period) {
