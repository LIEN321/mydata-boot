...
- 流水线任务
    - 新增“JS脚本”：编写自定义脚本，处理上下文数据；

### 优化

- 流水线任务

    - “删除数据”：增加过滤条件，以便删除指定范围的数据；

    - “JSON转数据”：字段映射支持[index]格式，用于将json数组转业务数据；

- API管理
    - 数据层级：支持${var}表达式 获取流水线上下文变量值；

### 修复

- 流水线
    - 禁用的流水线任务 在流水线执行后状态不准确问题；
...
- 流水线管理
    ...
    - 新增“异常重试次数”配置，流水线出现异常则重试；

- 流水线任务
    - 新增“异常重试次数”配置，单个任务出现异常则重试；

### 优化

- 流水线

    - 底层通过LiteFlow封装组件，流水线编排生成EL规则并执行；

    - 流水线连续失败由关闭定时和webhook 调整为禁用状态；

- 流水线任务

    - “发送邮件”任务：收件地址、主题、内容 支持${var}表达式，获取流水线上下文参数值；

    - “设置流水线参数”任务：原${var}表达式支持层级（ ${a.b.c} ）和基本运算（ ${var} + 1 ）；

    - “停止流水线”任务：is empty、is not empty条件支持对上下文的业务数据（BIZ_DATA）的判断；

    - 新增内置的“初始化”步骤，用于查看流水线执行前的准备内容；

### 修复

- 流水线

    - 修复 “发送邮件”任务出现异常未停止流水线问题；
...
- 应用管理
    - 认证配置，增加类型：JWT Bearer；
    - 新增接口成功规则配置，用于应用内API的全局规则；

- API管理
    - 新增成功规则配置，默认为响应码=200，支持判断响应json中的指定字段值；
...
- 应用管理
    - 增加“认证配置”，减少流水线内的认证操作，目前支持Cookie、API Key；
    - 应用授权范围在流水线单次执行内；
